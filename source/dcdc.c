...
int dcdc_ctrl_vdd_soc(int mv, bool step, bool wait, bool onlyUp) {
    ...
    if ((dcdc.reg_3 & BITNVAL(DCDC_REG3_BITS_TRG, DCDC_REG3_BITMASK_TRG)) == BITNVAL(DCDC_REG3_BITS_TRG, trg))
        return 0;

    if (onlyUp && ((dcdc.reg_3 & BITNVAL(DCDC_REG3_BITS_TRG, DCDC_REG3_BITMASK_TRG)) > BITNVAL(DCDC_REG3_BITS_TRG, trg)))
        return 0;
    ...
    printf("VDD_SOC %X => %X [%s]\n", DCDC_TRG_TO_VDD_SOC(dcdc.reg_3 & DCDC_REG3_BITMASK_TRG), mv, (step) ? "STEP" : "JUMP");
    dcdc.reg_3 = dcdc_r3;
    if (wait)

...
int glitch_configure(glitch_config_s* config, bool add_to_chain) {
    ...
    if (config->overrides.trigger.mask) {                                                                           // mask and expected masked value from trigger_data_reg
        ...
        if (uart_mode) {
            varray->trigger_ports = GLITCH_PAD_CTL_BITMASK_TEENSY_UART_WATERMARK | BITN(UART_DATA_BITS_RXEMPT);
            varray->trigger_exp_state = ((config->trigger_ctl >> GLITCH_PAD_CTL_BITS_TRIGGER_UART_WATERMARK) & GLITCH_PAD_CTL_BITMASK_TEENSY_UART_WATERMARK);
        } else if (init_trigger) {
            varray->trigger_ports = BITN(teensy_get_pad_port(trigger_pad));
            varray->trigger_exp_state = BITNVALM(teensy_get_pad_port(trigger_pad), !!(config->trigger_ctl & BITN(GLITCH_PAD_CTL_BITS_TRIGGER_STATE)), 1);
        }
    }
...
int glitch_configure_default(int type, uint32_t offset, uint32_t offset_mult, uint32_t width) {
    ...
    if (type & BITN(GLITCH_CONFIG_DEFAULT_TYPE_BITS_FLAG_NODRIVER))
        config.driver_ctl = BITN(GLITCH_PAD_CTL_BITS_ALT_MODE);
    else {
    ...
    if (type & BITN(GLITCH_CONFIG_DEFAULT_TYPE_BITS_FLAG_NOTRIGGER)) {
        config.trigger_ctl = BITN(GLITCH_PAD_CTL_BITS_ALT_MODE);
        ...
        case BITN(GLITCH_CONFIG_DEFAULT_TYPE_BITS_UART):
            config.trigger_ctl = GLITCH_DEFAULT_UART_TRIGGER_UARTN
                ...
                | BITN(GLITCH_PAD_CTL_BITS_ALT_MODE)
                ;
            ret = glitch_configure(&config, !!(type & BITN(GLITCH_CONFIG_DEFAULT_TYPE_BITS_FLAG_CHAIN)));
            if (ret >= 0) {
